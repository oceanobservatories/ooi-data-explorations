...
time_array=[];pco2w_array=[];temperature_array=[];
...
for i = 1:length(nc_urls)
    ...
    %Temperature
    data=ncread(char(nc_urls(i,:)),'pco2w_thermistor_temperature');
...
subplot(211)
plot(time_array,pco2w_array,'.k')
axis([datenum(2018,1,1) datenum(2019,1,1) 0 2000])
xticks(ticksx(ind))
xticklabels(datestr(ticksx(ind)))
title(strcat(mooring_name,{' '},node,{' '},'PCO2W'))
ylabel('uatm')
