...
m = m2m_request(site, node, sensor, method, 'vel3d_cd_dcl_system_data_recovered', start, stop)
system = m2m_collect(m, tag)  # provides heading, pitch, roll, and seawater temperature data
m = m2m_request(site, node, sensor, method, 'vel3d_cd_dcl_data_header_recovered', start, stop)
...
out_file = ('%s.%s.%s.deploy17.%s.%s.nc' % (site.lower(), level, instrmt, method,
                                            'vel3d_cd_dcl_velocity_data_recovered'))
