...
@brief Load the NUTNR data from the uncabled, Coastal Endurance Surface
...
def combine_delivery_methods(site, node, sensor, resample=180):
    ...
    the uncabled CTD (NUTNR), and combines each of them into a single, merged
    xarray data set.
    ...
    :return merged: the merged NUTNR data stream resampled to a user-defined
        time interval
    """
    # set the parameter names of interest
    parameters = ['corrected_nitrate_concentration', 'nitrate_concentration']
    ...
    rinst = suna_instrument(rinst)

    # use the original OOI QC results variables and vendor defined sensor tests to remove data that fail the QC tests
    for param in parameters:
        m = (telem[param + '_qc_summary_flag'] >= 4) | (telem['nitrate_sensor_quality_flag'] >= 4) | (telem[param] < 1)
        telem[param][m] = np.nan
        m = (rhost[param + '_qc_summary_flag'] >= 4) | (rhost['nitrate_sensor_quality_flag'] >= 4) | (rhost[param] < 1)
        rhost[param][m] = np.nan
        m = (rinst[param + '_qc_summary_flag'] >= 4) | (rinst['nitrate_sensor_quality_flag'] >= 4) | (rinst[param] < 1)
...
def generate_deq(site, node, sensor, resample, cut_off):
    """
    Load the Nitrate sensor data for a defined reference designator (using the
    site, node and sensor names to construct the reference designator)
    collected via the telemetered, recovered host and instrument methods and
    combine them into a single data set that can be used to create a data file
    for the DEQ water quality project.
...
def main(argv=None):
    ...
    out_path = os.path.join(os.path.expanduser('~'), 'ooidata/wqx/nutnr')
