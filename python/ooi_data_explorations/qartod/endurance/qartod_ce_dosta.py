    ...
    if node in ['SP001', 'WFP01']:
        ...
        if node == 'SP001':
            ...
            for grp in grps:
                ...
                deployments.append(dosta_wfp(grp[1]))
            deployments = [i for i in deployments if i]
            telem = xr.concat(deployments, 'time')

            print('##### Downloading the recovered_wfp DOSTA data for %s #####' % site)
...
def generate_qartod(site, node, sensor, cut_off):
    ...
    if node == 'WFP01':
        index = 15  # decimate the WFP data so we can process it
    else:
        index = 1
    ...
    gr_lookup = process_gross_range(data, parameters, limits, site=site,
                                    node=node, sensor=sensor, stream='dosta_sample')
    ...
    clm_lookup, clm_table = process_climatology(data, parameters, limits, depth_bins=depth_bins,
                                                site=site, node=node, sensor=sensor, stream='dosta_sample')
