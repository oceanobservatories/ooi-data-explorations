...
# GliderDAC internal names -> OOI stream variable names
_RENAME = {
...
def combine_delivery_methods(site: str, node: str, sensor: str):
    """
    Collect all CE glider CTD data from the GliderDAC near the
    deployment location of the given reference designator.

    Vocabulary is used to determine the latitude and longitude used
    as the center of the GliderDAC search. The returned dataset has
    variables renamed from GliderDAC internal names to the OOI stream
    variable names used in the QARTOD lookup tables.
    ...
    :return: xarray Dataset of all CE glider CTD data near the site
    """
    ...
    data = collect_glider(lat, lon, institution, variables=['temperature', 'conductivity', 'salinity', 'density'],
                          extent=100.0)
    data = data.rename({k: v for k, v in _RENAME.items() if k in data})
    return data
...
def generate_qartod(site: str, node: str, sensor: str, cut_off: str) -> tuple:
    ...
    parameters = ['sea_water_temperature', 'sea_water_electrical_conductivity',
                  'sea_water_practical_salinity']
    limits = [[-5, 35], [0, 9], [0, 42]]
    ...
    clm_lookup, clm_table = process_climatology(data, parameters[0:3:2], limits[0:3:2],
                                                depth_bins=depth_bins, site=site,
                                                node=node, sensor=sensor, stream=streams[0])
...
def main(argv=None):
    ...
    # annotations are empty for GliderDAC data, but save for consistency
