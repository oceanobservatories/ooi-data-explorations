...
from ooi_data_explorations.common import get_vocabulary, get_annotations, load_gc_thredds, add_annotation_qc_flags
from ooi_data_explorations.combine_data import combine_datasets
from ooi_data_explorations.uncabled.process_nutnr import suna_datalogger, suna_instrument, suna_cspp
from ooi_data_explorations.qartod.qc_processing import woa_standard_bins, identify_blocks, create_annotations, \
    process_gross_range, process_climatology, inputs, ANNO_HEADER, CLM_HEADER, GR_HEADER
...
def combine_delivery_methods(site, node, sensor):
    ...
    """
    # set the tag constants
    tag = '.*NUTNR.*\\.nc$'

    if node == 'SP001':
        # CSPP data sets
        print('##### Downloading the recovered_cspp NUTNR data for %s #####' % site)
        rinst = load_gc_thredds(site, node, sensor, 'recovered_cspp', 'nutnr_j_cspp_instrument_recovered', tag)
        deployments = []
        ...
        for grp in grps:
            ...
            deployments.append(suna_cspp(grp[1]))
...
def generate_qartod(site, node, sensor, cut_off):
    ...
    clm_lookup, clm_table = process_climatology(data, parameters, limits, depth_bins=depth_bins,
                                                site=site, node=node, sensor=sensor, stream='suna')
    ...
    clm_lookup['source'] = ('User range based on data collected through {}.'.format(src_date))
...
def main(argv=None):
    ...
    out_path = os.path.join(os.path.expanduser('~'), 'ooidata/qartod/nutnr')
