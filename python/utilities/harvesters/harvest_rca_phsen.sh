...
BASE_FILE="${HOME}/ooidata/m2m/ce04osbp/seafloor/phsen/ce04osbp.seafloor.phsen"
for i in $(seq -f "%02g" 1 6); do
  $PYTHON $BASE_FLAGS -mt streamed -st phsen_data_record -dp $i -o "$BASE_FILE.deploy$i.streamed.phsen_data_record.nc"
done
...
BASE_FILE="${HOME}/ooidata/m2m/ce04osps/200m/phsen/ce04osps.200m.phsen"
for i in $(seq -f "%02g" 1 6); do
  $PYTHON $BASE_FLAGS -mt streamed -st phsen_data_record -dp $i -o "$BASE_FILE.deploy$i.streamed.phsen_data_record.nc"
done

BASE_FLAGS="-s CE04OSPS -n SF01B -sn 2B-PHSENA108"
BASE_FILE="${HOME}/ooidata/m2m/ce04osps/profiler/phsen/ce04osps.profiler.phsen"
for i in $(seq -f "%02g" 1 6); do
  $PYTHON $BASE_FLAGS -mt streamed -st phsen_data_record -dp $i -o "$BASE_FILE.deploy$i.streamed.phsen_data_record.nc"
done
...
BASE_FILE="${HOME}/ooidata/m2m/rs01sbps/200m/phsen/rs01sbps.200m.phsen"
for i in $(seq -f "%02g" 1 6); do
  $PYTHON $BASE_FLAGS -mt streamed -st phsen_data_record -dp $i -o "$BASE_FILE.deploy$i.streamed.phsen_data_record.nc"
done

BASE_FLAGS="-s RS01SBPS -n SF01A -sn 2D-PHSENA101"
BASE_FILE="${HOME}/ooidata/m2m/rs01sbps/profiler/phsen/rs01sbps.profiler.phsen"
...
BASE_FILE="${HOME}/ooidata/m2m/rs03axps/200m/phsen/rs03axps.200m.phsen"
for i in $(seq -f "%02g" 1 5); do
  $PYTHON $BASE_FLAGS -mt streamed -st phsen_data_record -dp $i -o "$BASE_FILE.deploy$i.streamed.phsen_data_record.nc"
done

BASE_FLAGS="-s RS03AXPS -n SF03A -sn 2D-PHSENA301"
BASE_FILE="${HOME}/ooidata/m2m/rs03axps/profiler/phsen/rs03axps.profiler.phsen"
for i in $(seq -f "%02g" 1 5); do
  $PYTHON $BASE_FLAGS -mt streamed -st phsen_data_record -dp $i -o "$BASE_FILE.deploy$i.streamed.phsen_data_record.nc"
done
